...
def NACA_validation(t,m,p,c,Npoints=500):
    ...
    ###############################################
    print('Max analytical thickness = ',str(t),'\nMax numerical thickness = ',str(max_th.real))

    max_th_index = np.where(section_thickness_dist[1]==max_th)
    print('\n\nPosition of max analytical thickness = ',str(x_mxth),'\nPosition of max numerical thickness = ',str(section_thickness_dist[0][max_th_index].real))
    ...
    for i,th_i in enumerate(section_thickness_dist[1]):
        ...
        errorDirect[i] = abs(th_i-evalTh)/abs(evalTh)
    ...
    ax4.set_ylabel(r'$\frac{|Thx)-Th_{Num}(x)|}{|Th(x)|}$', color='k', labelpad=12)
...
def main():
    ...
    for i in range(len(t)):
        NACA_validation(t[i],m[i],p[i],c[i])
